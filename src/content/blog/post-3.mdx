...
I created a demo to test how this behaves when zooming in. You can zoom in and change the window width to see how the font-size changes. The font-size is displayed in the bottom left corner. The visual font-size is the font-size multiplied with the zoom level percentage.

import Static from "../../components/Demos/ResponsiveType/Static.svelte";

<Static client:idle />

Try zooming in with a viewport width of 1680px. At first everything works fine, the
...
write a media query for each zoom level and downscale the font-size value so that
multiplied with the zoom level the visual font-size wouldn't appear to change at
...
rather complex to test. You can see for yourself how it behaves with a larger window width.
