...
Fluid typography isn’t new. Many great articles and tools already cover it. Today, you can find multiple generators that spit out the necessary CSS, or plugins like Utopia that integrate fluid scales into your build process. Under the hood, though, they all rely on the same [formula](https://www.smashingmagazine.com/2022/01/modern-fluid-typography-css-clamp/#fluid-sizing-function) first outlined by Adrian Bece. These solutions work well, but depend on external tooling or a build step. That made me curious: could I take the math and express the whole thing directly in CSS? No Sass, no generators, just variables, `calc()`, and `clamp()`.
