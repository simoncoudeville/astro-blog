...
Fluid typography has been explored in detail by others before me. Smashing Magazine showed how to use Sass functions to handle the math behind clamp(), so you don’t have to hard-code awkward formulas by hand. Utopia's plugin generates ready to use fluid custom properties and even introduced a fluid type scale. These approaches work beautifully but they depend on build tools. That made me curious: could I take the math and express the whole thing directly in CSS? No Sass, no generators, just variables, calc(), and clamp().
...
The exact calculation is rather complex. If you want to know how it works you should read Adrian Bece's article on [Modern Fluid Typography Using CSS Clamp](https://www.smashingmagazine.com/2022/01/modern-fluid-typography-css-clamp/). I somehow managed to work it out myself and found that with all the parameters mentioned above, this is the calculation that's needed to get the fluid formula:
...
I just wanted to see if it was possible to express the whole thing in pure CSS. Turns out, it is. The main advantage is that you don't need any extra tools or build steps to achieve this. Just CSS, which I personally find quite elegant.