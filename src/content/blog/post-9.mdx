...
Fluid typography isn’t new. Many great articles and tools already cover it. These days, you can find online tools that generate the necessary CSS, or plugins like Utopia that integrate fluid scales into your build process. Under the hood, though, they all rely on the same [formula](https://www.aleksandrhovhannisyan.com/blog/fluid-type-scale-with-css-clamp/) by Aleksandr Hovhannisyan. These solutions work well, but depend on external tooling or a build step. That made me curious: could I take the math and express the whole thing directly in CSS? No Sass, no generators, just variables, `calc()`, and `clamp()`.
...
These values didn't happen by chance. It's perfectly possible to calculate the exact values for the two breakpoints. That's where Adrian's formula comes in. To get to the formula you only need these 4 parameters:
