...
Fluid typography isn’t new. Many great articles and tools already cover it. These days, you can find multiple generators that generate the necessary CSS, or plugins like Utopia that integrate fluid scales into your build process. Under the hood, though, they all rely on the same [formula](https://www.aleksandrhovhannisyan.com/blog/fluid-type-scale-with-css-clamp/). These solutions work well, but depend on external tooling or a build step. That made me curious: could I take the math and express the whole thing directly in CSS? No Sass, no generators, just variables, `calc()`, and `clamp()`.
