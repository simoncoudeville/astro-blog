...
Fluid typography isn’t new. Many great articles and tools already cover it. There are multiple online tools that generate the necessary CSS. Plugins like Utopia integrate fluid scales into your build process. These solutions work well, but depend on external tooling or a build step. And under the hood they all rely on the same [formula](https://www.aleksandrhovhannisyan.com/blog/fluid-type-scale-with-css-clamp/) by Aleksandr Hovhannisyan. That made me curious: could I take the math and express the whole thing directly in CSS? No Sass, no generators, just variables, `calc()`, and `clamp()`.
...
These values didn't happen by chance. It's perfectly possible to calculate the exact values for the two breakpoints. That's where Aleksandr's formula comes in. To get to the formula you only need these 4 parameters:
