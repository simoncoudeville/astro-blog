...
published: false
description: Snapping images to a baseline grid used to require JavaScript. With container queries, advanced attr() and round(), it can now be done in pure CSS.
tags: css, webdev, frontend, typography
canonical_url: https://simoncoudeville.be/blog/aligning-images-to-a-baseline-grid-with-modern-css/
# cover_image: https://...
---
...
:root {
  ...
  --whitespace-1: var(--root-baseline);
  --whitespace-2: calc(2 * var(--root-baseline));
  --whitespace-3: calc(3 * var(--root-baseline)); /* == 1rlh */
  /* ...and so on */
...
This is especially useful for fluid typography, where the font size changes with the viewport width, so the line height needs to keep up with it. You can see how it works in [this codepen](https://codepen.io/simoncoudeville/pen/vYPYWWm).

## The image technique

The height of an image is whatever the aspect ratio makes it at the current width, so it can never land on the grid, and it pushes everything after it off rhythm. Until recently the only fix was JavaScript: measure the rendered width, work out the height that would snap to the grid, set it inline, and redo all of that on every resize. Now the whole thing can be done in CSS. Just like the text line-height rounding, it is possible to round the image height up to the next line. The only thing we need is the image's width and aspect ratio, which is where advanced `attr()` and container queries come in.
...
Have a look at a working example on the [demo page](https://simoncoudeville.be/demos/baseline-grid/) with the latest version of Chrome or Edge. Or checkout the [codepen](https://codepen.io/simoncoudeville/pen/rajaaQg).
